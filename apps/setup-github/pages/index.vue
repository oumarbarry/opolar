...
const manifest = ref("")
const isGitHubCodespace = ref(false)
...
onMounted(() => {
  ...
  if (codespaceName) {
    // host = `https://${codespaceName}.${codespacesPortForwardingDomain}`
    ingressHost = `https://${codespaceName}-8080.${githubCodespacesPortForwardingDomain}`

    isGitHubCodespace.value = true
  }

  manifest.value = JSON.stringify({
    // Redirect URL in the github app manifest setup flow. Will not be a part of the app.
    redirect_url: `${window.location.protocol}//${window.location.host}/callback`,

    name: `polar-${codespaceName}`.substring(0, 32),
    url: "http://localhost",
    hook_attributes: {
...
<template>
  <main v-if="!manifest" class="flex min-h-screen flex-col  gap-12 p-24">
    Loading...
  </main>
  <main v-else class="flex min-h-screen flex-col  gap-12 p-24">
    ...
    <div class="text-gray-800">
      Polar needs a GitHub Application to run. Follow this guide to create one in one click!
    </div>
    ...
    <div v-if="isGitHubCodespace">
      You&apos;re running on GitHub Codespace, this should work out of the box!
    </div>
    <div v-else>
